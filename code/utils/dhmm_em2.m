function [LL, prior, transmat, obsmat, nrIterations] = ...
   dhmm_em2(data, prior, transmat, obsmat, varargin)
...
% 'obs_prior_weight' - weight to apply to dirichlet prior on observation matrix [0]
% 'ini_prior_weight' - weight to apply to dirichlet prior on initial state vector [0]
% 'tran_prior_weight' -  weight to apply to dirichlet prior on transition matrix [0]
%
...
[max_iter, thresh, verbose, obs_prior_weight, tran_prior_weight, ini_prior_weight, adj_prior, adj_trans, adj_obs] = ...
   process_options(varargin, 'max_iter', 10, 'thresh', 1e-4, 'verbose', 1, ...
   'obs_prior_weight', 0, 'tran_prior_weight', 0, 'ini_prior_weight', 0, 'adj_prior', 1, 'adj_trans', 1, 'adj_obs', 1);
...
while (num_iter <= max_iter) & ~converged
 ...
 [loglik, exp_num_trans, exp_num_visits1, exp_num_emit] = ...
     compute_ess_dhmm(prior, transmat, obsmat, data, obs_prior_weight, tran_prior_weight, ini_prior_weight);
...
function [loglik, exp_num_trans, exp_num_visits1, exp_num_emit, exp_num_visitsT] = ...
   compute_ess_dhmm(startprob, transmat, obsmat, data, dirichlet_obs, dirichlet_tran, dirichlet_ini)
...
numex = length(data);
[S O] = size(obsmat);
exp_num_trans = dirichlet_tran + zeros(S,S);
exp_num_visits1 = dirichlet_ini + zeros(S,1);
exp_num_visitsT = zeros(S,1);
exp_num_emit = dirichlet_obs + zeros(S,O);
loglik = 0;
